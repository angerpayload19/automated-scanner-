...
def helper():
        ...
        print("------------")
        print("\t./INSA.py example.com: Scans the domain example.com.")
        print("\t./INSA.py example.com --skip dmitry --skip theHarvester: Skip the 'dmitry' and 'theHarvester' tests.")
        print("\t./INSA.py --help     : Displays this help context.")
        print(bcolors.OKBLUE+"Interactive:"+bcolors.ENDC)
        print("------------")
        print("\tCtrl+C: Skips current test.")
        print("\tCtrl+Z: Quits INSAScan.")
...
|___|_| \_|____/_/   \_\
 ------------------------                   
                     ...
                     Author """+bcolors.BG_LOW_TXT+"""BERNABAS MELES"""+bcolors.ENDC+""" 
    """
    print(logo_ascii)
    print(bcolors.ENDC)
...
class Spinner:
    ...
    def spinner_task(self):
        ...
        try:
            ...
            print("\n\t"+ bcolors.BG_ERR_TXT+"INSAscan received a series of Ctrl+C hits. Quitting..." +bcolors.ENDC)
            sys.exit(1)
...
tool_cmd   = [
                ...
                #2
                ["wget -O /tmp/INSAscan_temp_aspnet_config_err --tries=1 ","/%7C~.aspx"],

                #3
                ["wget -O /tmp/INSAscan_temp_wp_check --tries=1 ","/wp-admin"],

                #4
                ["wget -O /tmp/INSAscan_temp_drp_check --tries=1 ","/user"],

                #5
                ["wget -O /tmp/INSAscan_temp_joom_check --tries=1 ","/administrator"],
                ...
                #75
                ["wget -O /tmp/INSAscan_temp_aspnet_elmah_axd --tries=1 ","/elmah.axd"],
                ...
                #78
                ["wapiti "," -f txt -o INSAscan_temp_wapiti"],
...
if args_namespace.help or (not args_namespace.update \
    ...
    logo()
    print("INSA is updating....Please wait.\n")
    spinner.start()
    ...
    if rs_internet_availability == 0:
        ...
        spinner.stop()
        sys.exit(1)
    cmd = 'sha1sum INSA.py | grep .... | cut -c 1-40'
    oldversion_hash = subprocess.check_output(cmd, shell=True)
    oldversion_hash = oldversion_hash.strip()
    os.system('wget -N bernabasmeles123@gmail.com -O INSAscan.py > /dev/null 2>&1')
    ...
    if oldversion_hash == newversion_hash :
        clear()
        print("\t"+ bcolors.OKBLUE +"You already have the latest version of INSAscan." + bcolors.ENDC)
    else:
        clear()
        print("\t"+ bcolors.OKGREEN +"INSAScan successfully updated to the latest version." +bcolors.ENDC)
    spinner.stop()
    sys.exit(1)
    ...
    while (rs_avail_tools < len(tools_precheck)):
        ...
        try:
            ...
            print("\t"+bcolors.BG_ERR_TXT+"INSAscan was terminated abruptly..."+bcolors.ENDC)
            sys.exit(1)
    ...
    if len(unavail_tools_names) == 0:
        print("\t"+bcolors.OKGREEN+"All Scanning Tools are available. Complete vulnerability checks will be performed by INSAscan."+bcolors.ENDC)
    else:
        print("\t"+bcolors.WARNING+"Some of these tools "+bcolors.BADFAIL+str(unavail_tools_names)+bcolors.ENDC+bcolors.WARNING+" are unavailable or will be skipped. INSAscan will still perform the rest of the tests. Install these tools to fully utilize the functionality of INSAscan."+bcolors.ENDC)
    ...
    while(tool < len(tool_names)):
        ...
        temp_file = "/tmp/INSAscan_temp_"+tool_names[tool][arg1]
        ...
        if runTest == 1:
                ...
                print("\n"+bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit INSAscan.\n" + bcolors.ENDC)
                rs_skipped_checks = rs_skipped_checks + 1
    ...
    if len(rs_vul_list)==0:
        ...
        with open("INSA-Vulnerability-Report", "a") as report:
            while(rs_vul < len(rs_vul_list)):
                vuln_info = rs_vul_list[rs_vul].split('*')
                report.write(vuln_info[arg2])
                report.write("\n------------------------\n\n")
                temp_report_name = "/tmp/INSAscan_temp_"+vuln_info[arg1]
            ...
            print("\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+"`INSA-Vulnerability-Report`"+bcolors.ENDC+" is available under the same directory INSAScan resides.")

        report.close()
    
    for file_index, file_name in enumerate(tool_names):
        with open("INSA-Debug-ScanLog", "a") as report:
            try:
                with open("/tmp/INSAscan_temp_"+file_name[arg1], 'r') as temp_report:
                        data = temp_report.read()
    ...
    print("\n")
    print("\tFor Debugging Purposes, You can view the complete output generated by all the tools named "+bcolors.OKBLUE+"`INSA-Debug-ScanLog`"+bcolors.ENDC+" under the same directory.")
    print(bcolors.BG_ENDL_TXT+"[ Report Generation Phase Completed. ]"+bcolors.ENDC)

    os.system('setterm -cursor on')
    os.system('rm /tmp/INSAscan_te* > /dev/null 2>&1') 
